...
#' @param recipeGraph
#' @param products
clean_binary_lp_results <- function(lp_result = binary_LP_result, recipeData =
                                    current_recipes$data_frame, recipeGraph =
                                    current_recipes$graph, products =
                                    Opt_products) {
  ...
  itemConsumptionMatrix <- t(make_recipeMatrix(names(products), recipeGraph = recipeGraph, recipeData = recipeData, itemMode = "component")) * lp_result$solution
