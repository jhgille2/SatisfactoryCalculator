...
tar_load(c(CytoscapeReady_binary,
           current_recipes,
           Opt_products))
...
# I'll solve this first assuming no alternate recipes and then work on
# a solution to the alternates later
solution_reduced <- solution_df %>% 
  select(from, to, recipe, flow, product_amt, product_per_minute)

# Look at the first item that is consumed in the solution
first_item <- solution_df[1, ]
first_item

# What recipes in the solution make the item that is being consumed here?
first_item_recipe <- solution_df %>%
  dplyr::filter(to == first_item$from[[1]])
first_item_recipe
...
num_factories           <- first_item$flow/first_item_recipe$product_per_minute
...
item_factories <- tibble(factory_name = paste(first_item_recipe$recipe, "to", first_item$recipe, sep = "_"),
                         nFactories   = total_factories) %>% 
  uncount(nFactories) %>% 
  mutate(factory_name     = paste(factory_name, 1:n(), sep = "_"),
         factory_recipe   = first_item_recipe$recipe,
         factory_target   = first_item$recipe,
         product_name     = first_item$from[[1]],
         underclock_value = underclock_vec,
         product_qty      = first_item_recipe$product_per_minute[[1]]*underclock_value,
         splitter_node = paste("splitter", factory_name, sep = "_"),
         merger_node   = paste("merger", factory_name, sep = "_"),
         is_last_factory  = c(rep(F, nrow(.)-1), T))

# This does not account for the factories of the final products, so I'll handle
# them separately
end_products <- solution_df$to[!(solution_df$to %in% solution_df$from)] %>% unique()

solution_df %>% 
  dplyr::filter(to %in% end_products) %>% 
  select(recipe, product_amt, product_per_minute, to) %>%
  distinct() %>% 
  group_by(recipe) %>% 
  ...
  mutate(splitter_node = paste("splitter", factory_name, sep = "_"),
         merger_node   = paste("merger", factory_name, sep = "_"),
         is_last_factory  = ifelse(nrow(.) > 1, c(rep(F, nrow(.)-1), T), T),
         is_first_factory = ifelse(nrow(.) > 1, c(T, rep(F, nrow(.)-1)), T),
...
ore_df <- current_recipes$data_frame %>% 
  dplyr::filter(producedIn == "_Converter_C") 
...
bind_rows(tibble(input = "source_node",
                 product = "Ore",
                 factory_type = "environment"))
...
factory_graph <- igraph::graph_from_data_frame(all_connections, vertices = node_df)

RCy3::createNetworkFromIgraph(factory_graph)
